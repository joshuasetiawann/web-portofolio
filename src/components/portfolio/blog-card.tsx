...
export function BlogCard({ post }: BlogCardProps) {
  return (
    <article
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-surface-1",
    ...
    >
      ...
      >
        {post.cover ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-primary/25 via-surface-2 to-accent/20">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <ArrowUpRight
          className="absolute top-5 right-5 size-5 text-foreground-subtle opacity-0 transition-opacity group-hover:opacity-100"
          aria-hidden="true"
        />
      </div>
    </article>
